...
from .midi import CoreMidiProcessTransport, MidoMidiTransport, MemoryMidiTransport
...
def build_parser() -> argparse.ArgumentParser:
    ...
    serve_parser.add_argument(
        "--coremidi-helper",
        help="path to the CoreMIDI C helper; avoids python-rtmidi on macOS",
    )
    ...
    check_parser.add_argument("--coremidi-helper", help="path to the CoreMIDI C helper")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "check":
        if args.coremidi_helper:
            output_name = args.midi_output or "Mixxx API Bridge In"
            input_name = args.midi_input or "Mixxx API Bridge Out"
        elif not args.midi_output or not args.midi_input:
            print("check requires --midi-output and --midi-input (or --coremidi-helper)", file=sys.stderr)
            return 2
        try:
            transport = (
                CoreMidiProcessTransport(args.coremidi_helper, output_name, input_name)
                if args.coremidi_helper
    ...
    if args.command == "serve":
        ...
        elif args.coremidi_helper:
            output_name = args.midi_output or "Mixxx API Bridge In"
            input_name = args.midi_input or "Mixxx API Bridge Out"
            try:
                transport = CoreMidiProcessTransport(args.coremidi_helper, output_name, input_name)
    ...
    if args.command == "send":
        if not args.coremidi_helper and not args.midi_output:
            print("send requires --midi-output, or use --coremidi-helper", file=sys.stderr)
            return 2
